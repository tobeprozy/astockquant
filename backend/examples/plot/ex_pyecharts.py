...
# 初始化接口
print("Tushare 接口调用正常，版本号:" + ak.__version__ +"\n")
...
class Stock:
    ...
    def set_code(self,code):
        ...
        stock_list = ak.fund_name_em()
        a = stock_list["基金代码"].tolist()
 
        for i in range(len(a)):
            if a[i][0:6] == code:
                self.code=a[i]
                self.name=stock_list.loc[i,"基金简称"]
            # 请求股票列表
    ...
    def get_daily_trade(self):  # 股票代码，开始日期20120101，结束日期20180101
        ...
        # df=ak.fund_etf_hist_em(symbol=self.code, period="daily", start_date=self.start.strftime('%Y%m%d'), end_date=self.end.strftime('%Y%m%d'), adjust="")

        start_date = datetime.datetime.strptime(self.start, '%Y%m%d').date()
        end_date = datetime.datetime.strptime(self.end, '%Y%m%d').date()

        # 现在可以安全地调用 strftime 方法
        df = ak.fund_etf_hist_em(symbol=self.code, period="daily", 
    ...
    def Print_Main_index(self,kline,bar_volumn,line_ma=None,line_ma2=None,line_ma3=None,line_ma4 = None,itheme="light"):
        ...
        grid_chart.render(path = "./StockAnalysis/pyecharts-{}-{}.html".format(self.code[0:6],self.name))
        
        print("已保存文件至{}".format("./StockAnalysis/pyecharts-{}-{}.html".format(self.code[0:6],self.name)))
        os.system("start ./Desktop/pyecharts-{}-{}.html".format(self.code[0:6],self.name))
...
class ExcelWriter:
    ...
    def saveExcel(self):
        ...
        import os
        os.system("start  ./StockAnalysis/totalAnalysis.xlsx")
...
if __name__ == '__main__':
    ...
    for i in codeList:
        stock1 = Stock()
        stock1.set_code(i)
        stock1.start = "20241001"
        stock1.end = "20241207"
        stock1.begin()
    ew = ExcelWriter()
    ew.setLocation("./StockAnalysis/totalAnalysis.xlsx")
    ew.newFile()
