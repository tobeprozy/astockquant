...
def main(code="600733", start_cash=100000, stake=100, commission_fee=0.001):
    cerebro = bt.Cerebro()  # 创建主控制器
    cerebro.optstrategy(MyStrategy, maperiod=range(3, 31))  # 导入策略参数寻优
    # 利用 AKShare 获取股票的后复权数据，这里只获取前 7 列
    stock_hfq_df = ak.stock_zh_a_hist(symbol=code, adjust="hfq", start_date='20240101', end_date='20240607').iloc[:, :7]
    # 删除 `股票代码` 列
    ...
    start_date = datetime(2024, 1, 2)  # 回测开始时间
    end_date = datetime(2024, 12, 6)  # 回测结束时间
    ...
    import matplotlib
    # 设置字体
    matplotlib.rcParams['font.sans-serif'] = ['DejaVu Sans']  # 替代字体
...
if __name__ == '__main__':
    main(code="600733", start_cash=100000, stake=100, commission_fee=0.001)
